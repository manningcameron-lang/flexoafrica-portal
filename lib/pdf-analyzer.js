...
function mergeInkSeparations(mathSeparations, ink) {
  ...
  for (let i = 0; i < ink.separations.length; i++) {
    ...
    // Preview priority (best to worst):
    //   1. analyser preview — accurate Ghostscript render of THIS separation,
    //      works for CMYK AND spot colours
    //   2. math preview — client-side approximation from the composite (only
    //      ever exists for CMYK, may not match the actual ink lay)
    //   3. null — tile falls back to a "Spot" text card
    const analyserPreview =
      typeof s.preview_png_base64 === "string" && s.preview_png_base64.length > 0
        ? s.preview_png_base64
        : null;
    ...
    out.push({
      ...
      previewDataUrl: analyserPreview || mathMatch?.previewDataUrl || null,
    });
  }
  return out;
}
