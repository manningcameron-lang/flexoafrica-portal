...
export async function createOrderFromPortal({ profile, header, lineItems }) {
  ...
  for (const li of lineItems) {
    ...
    let pdfMeta = null;
    if (li.pdfFile) {
      // Use the first job's id as the "owner" of the PDF path for storage rules.
      const firstJobId = doc(collection(db, "jobs")).id;
      pdfMeta = await uploadJobPdf({ jobId: firstJobId, file: li.pdfFile });
      // Stash the firstJobId so we can use it as the first job below.
      pdfMeta.__firstJobId = firstJobId;
    }
    ...
    for (let plateIdx = 0; plateIdx < platesToCreate.length; plateIdx++) {
      ...
      let jobRef;
      if (plateIdx === 0 && pdfMeta && pdfMeta.__firstJobId) {
        jobRef = doc(collection(db, "jobs"), pdfMeta.__firstJobId);
      } else {
      ...
      await setDoc(jobRef, {
        ...
        pdfUrls: pdfMeta ? [{ ...pdfMeta, __firstJobId: undefined }] : [],
