...
// ──────────────────────────────────────────────────────────────────────────
// DEMO MODE
// While the portal is still being built, every portal-created order/job
// is flagged demo:true, allocated from a SEPARATE counter, and uses
// distinctive prefixes (PDO / PD) so they cannot be confused with real FA
// work tickets. The MIS dashboard filters demo:true jobs out so operators
// don't see test orders pulling through.
//
// To go live: set PORTAL_DEMO_MODE = false. Real customers' orders will
// then use the real `orderNumber` / `jobNumber` counters and show up in
// the MIS dashboard with the PORTAL badge.
// ──────────────────────────────────────────────────────────────────────────
export const PORTAL_DEMO_MODE = true;

const ORDER_COUNTER_KEY = PORTAL_DEMO_MODE ? "demoOrderNumber" : "orderNumber";
const JOB_COUNTER_KEY = PORTAL_DEMO_MODE ? "demoJobNumber" : "jobNumber";
const ORDER_PREFIX = PORTAL_DEMO_MODE ? "PDO" : "FAO";
const JOB_PREFIX = PORTAL_DEMO_MODE ? "PD" : "FA";

export function formatJobNumber(n) {
  return JOB_PREFIX + String(n).padStart(6, "0");
}
...
export function formatOrderNumber(n) {
  return ORDER_PREFIX + String(n).padStart(6, "0");
}
...
export async function createOrderFromPortal({ profile, header, lineItems }) {
  ...
  // Allocate a human-friendly order number from the DEMO counter while
  // PORTAL_DEMO_MODE is on. Falls back to the real counter when flipped off.
  const orderNumberInt = await nextNumber(ORDER_COUNTER_KEY);
  ...
  const orderRef = await addDoc(collection(db, "orders"), {
    ...
    source: "portal",
    // Demo flag — MIS dashboard filters these out so they don't appear in
    // the operator queue while the portal is still being built.
    demo: PORTAL_DEMO_MODE,
  ...
  for (const li of lineItems) {
    ...
    for (let plateIdx = 0; plateIdx < platesToCreate.length; plateIdx++) {
      ...
      // Demo job numbers come from the separate counter so real FA work
      // tickets aren't burned by portal test orders.
      const jobNumberInt = await nextNumber(JOB_COUNTER_KEY);
      ...
      await setDoc(jobRef, {
        ...
        source: "portal",
        // Demo flag — MIS dashboard hides demo:true jobs while portal is
        // still in build. Flip PORTAL_DEMO_MODE off to ship for real.
        demo: PORTAL_DEMO_MODE,
        orderId: orderRef.id,
        orderNumber,
        customerUid: profile.uid,
